// Dados de todas as borboletas com fotos, onde encontrar e curiosidades
const especies = [
  {
...
// Função para gerar o HTML do guia dinamicamente numa nova aba
function abrirGuiaEmNovaAba() {
  ...
  novaAba.document.write(`
    ...
    <head>
      ...
      <style>
        .guia-container { max-width: 1100px; margin: 20px auto; padding: 20px; z-index: 2; }
        .guia-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 25px; margin-top: 30px; }
        .especie-card { background: var(--white-translucent, rgba(255, 255, 255, 0.9)); border-radius: 15px; padding: 20px; border: 1px solid rgba(35, 78, 53, 0.2); box-shadow: 0 8px 20px rgba(0,0,0,0.08); text-align: center; }
        .especie-card img { width: 100%; height: 200px; object-fit: cover; border-radius: 10px; border: 2px solid rgba(255, 255, 255, 0.8); }
        .especie-card h2 { font-family: 'Cormorant Garamond', serif; color: var(--green-dark, #1b3d29); font-size: 1.6rem; margin: 15px 0 10px 0; }
        .info-item { font-size: 0.9rem; text-align: left; line-height: 1.5; margin-bottom: 10px; color: #2b332c; }
        .back-btn { display: inline-block; margin-bottom: 20px; padding: 10px 20px; background: var(--green-dark, #1b3d29); color: #fff; text-decoration: none; border-radius: 20px; font-weight: 600; cursor: pointer; }
      </style>
    </head>
    <body>
